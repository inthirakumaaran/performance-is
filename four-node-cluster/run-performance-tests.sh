...
function after_execute_test_scenario() {
    ...
    ssh "$wso2is_4_host_alias" "IS_PID=\$(pgrep -u ubuntu java | head -1) && JCMD=\$(dirname \$(readlink -f /proc/\${IS_PID}/exe))/jcmd && echo \"Dumping JFR for PID: \${IS_PID}\" && \${JCMD} \${IS_PID} JFR.dump name=perf-recording filename=${jfr_file}" \
